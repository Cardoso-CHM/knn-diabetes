...
def cos(test_id,train_id): 
    # considere o objeto de teste (data[test_id]) como "a" e o objeto de treino (data[train_id]) como "b"
    a = data[test_id]
    b = data[train_id]
    
    numerator = sum(a * b) #somatorio: a[0]*b[0] + a[1]*b[1] + ... + a[n]*b[n]
    
    norm_A = sum(pow(a,2)) # somatorio: a[0]² + a[1]² + ... + a[n]²
    norm_B = sum(pow(b,2))# somatorio: b[0]² + b[1]² + ... + b[n]²
    
    denominator = math.sqrt(norm_A) * math.sqrt(norm_B) # raizQuadradaDe(a) * raizQuadradaDe(b)
...
def kNN(n):
    ...
    for x in test_ids:
        ...
        aux = aux[aux[:,0].argsort()][::-1][:n] #aplicando agsort decrescente (aux[aux[:,0].argsort()][::-1]) e pegando os n prirmeiros objetos ([:n])
...
while True:
    n = input("Type the size of neightbors (zero will terminate the program): ")
    n = int(n)
    if n == 0:  break
    res = kNN(int(n))
